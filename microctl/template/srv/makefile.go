...
URL := 348681422678.dkr.ecr.ap-southeast-1.amazonaws.com/backend/{{.Service}}
endif
...
deploy: docker
	helm upgrade --install {{.Service}} ./deploy/helm/{{.Service}} --set image.tag=$(TAG),image.repository=$(URL) -n $(NS)
...
k8s:
	...
	kubectl set image deployment/{{.Service}} {{.Service}}=$(URL):$(TAG)
	rm {{.Service}}
