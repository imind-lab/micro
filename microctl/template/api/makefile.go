...
ifndef IMAGE_URL
IMAGE_URL = registry.cn-beijing.aliyuncs.com/imind/{{.Name}}{{.Suffix}}
endif
...
ifndef RPC_HOST
RPC_HOST := {{.Name}}{{.Suffix}}.imind.tech
endif

{{.Package}}_names:={{.Package}}_api
...
build:
ifeq ($(LOCAL), false)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -o {{.Name}}{{.Suffix}} ./main.go
else
	CGO_ENABLED=0 GOOS=linux GOARCH=$(GOARCH) go build -a -installsuffix cgo -o {{.Name}}{{.Suffix}} ./main.go
endif
...
deploy:
	helm upgrade --install {{.Name}}{{.Suffix}} ./deploy/helm/{{.Name}}{{.Suffix}} --set image.repository=$(IMAGE_URL),image.tag=$(IMAGE_TAG),traefik.host=$(RPC_HOST) -n $(NAMESPACE)
