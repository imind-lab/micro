...
enum CreateGoogleHttpCode {
  ...
  // a ${backtick}CreateGoogleHttpStatus${backtick} value received from another address space belongs to
  // an error space that is not known in this address space.  Also
  // errors raised by APIs that do not return enough error information
  // may be converted to this error.
  //
  // HTTP Mapping: 500 Internal Server CreateGoogleHttpError
  UNKNOWN = 2;
...
message ResourceInfo {
  ...
  // error is [google.rpc.CreateGoogleHttpCode.PERMISSION_DENIED][google.rpc.CreateGoogleHttpCode.PERMISSION_DENIED].
...
// The ${backtick}CreateGoogleHttpStatus${backtick} type defines a logical error model that is suitable for different
...
// # Overview
//
// The ${backtick}CreateGoogleHttpStatus${backtick} message contains three pieces of data: error code, error message,
// and error details. The error code should be an enum value of
// [google.rpc.CreateGoogleHttpCode][google.rpc.CreateGoogleHttpCode], but it may accept additional error codes if needed.  The
...
// # Language mapping
//
// The ${backtick}CreateGoogleHttpStatus${backtick} message is the logical representation of the error model, but it
// is not necessarily the actual wire format. When the ${backtick}CreateGoogleHttpStatus${backtick} message is
...
// # Other uses
//
// The error model and the ${backtick}CreateGoogleHttpStatus${backtick} message can be used in a variety of
...
//     it may embed the ${backtick}CreateGoogleHttpStatus${backtick} in the normal response to indicate the partial
//     errors.
//
// - Workflow errors. A typical workflow has multiple steps. Each step may
//     have a ${backtick}CreateGoogleHttpStatus${backtick} message for error reporting.
//
// - Batch operations. If a client uses batch request and batch response, the
//     ${backtick}CreateGoogleHttpStatus${backtick} message should be used directly inside batch response, one for
//     each error sub-response.
//
// - Asynchronous operations. If an API call embeds asynchronous operation
//     results in its response, the status of those operations should be
//     represented directly using the ${backtick}CreateGoogleHttpStatus${backtick} message.
//
// - Logging. If some API errors are stored in logs, the message ${backtick}CreateGoogleHttpStatus${backtick} could
//     be used directly after any stripping needed for security/privacy reasons.
message CreateGoogleHttpStatus {
  // The status code, which should be an enum value of [google.rpc.CreateGoogleHttpCode][google.rpc.CreateGoogleHttpCode].
  int32 code = 1;

  // A developer-facing error message, which should be in English. Any
  // user-facing error message should be localized and sent in the
  // [google.rpc.CreateGoogleHttpStatus.details][google.rpc.CreateGoogleHttpStatus.details] field, or localized by the client.
  string message = 2;
