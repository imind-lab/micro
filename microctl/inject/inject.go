...
func (i *Inject) processResponse(line []byte) {
	...
	if temp == "}" {
		...
		if len(items) >= 2 {
			i.body.WriteString("\nfunc (x *" + items[1] + ") SetCode(code status.Code, msg ...string) {\n\tx.Code = int32(code)\n\tif len(msg) == 0 {\n\t\tx.Msg = code.String()\n\t} else {\n\t\tx.Msg = strings.Join(msg, \"，\")\n\t}\n}\n")
		}
		if len(items) >= 4 {
			i.body.WriteString("\nfunc (x *" + items[1] + ") SetBody(code status.Code, " + items[3] + " " + items[2] + ") {\n\tx.Code = int32(code)\n\tx.Msg = code.String()\n\tx." + cases.Title(language.English).String(items[3]) + " = " + items[3] + "\n}\n")
		}
		i.response = ""
	}
}
...
func (i *Inject) process(line []byte) {
	...
	} else if strings.Contains(temp, "//+IMind:tag") {
		i.tag = temp

		i.body.Write(line)
		i.body.WriteString("\n")
	} else if strings.Contains(temp, "//+IMind:response") {
		i.exist = true
		i.response = temp
